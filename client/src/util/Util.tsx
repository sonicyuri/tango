...
export class Util {
	...
	{
		let formatted = parts.filter(p => p.enabled || p.enabled === undefined).map(p => `${p.key}=${encodeURIComponent(p.value)}`).join("&");
	...
	{
		const queryString = Util.formatQueryString([{ key: "q", value: query || "", enabled: Boolean(query) }]);
